...
DM
- EBM is closely related to [[Diffusion Models]]
...
- Why gradient of log? (see/find above)
...
- Why we can't use the same model if we train it to predict the noise?
	...
	- Все что мы можем это только сэмплировать из зашумленного распределения
...
- ChatGPT
	...
	- Алгоритмически (как используется предсказание)
		- предсказание – глобальное
		- обновление – локальное
...
DM as VAE
- Thinking about [[Diffusion Models]] as hierarchical [[Variational Autoencoder (VAE)]]
- Encoder
	- Encoder in this case is just a Markovian process of adding Gaussian Noise
	- Nothing to learn in Encoder
- Scaling for the mean and variance
	- Do we use scaling in Denoising score matching?
	- Variance: YES, we are regulating the amount of added noise that way
	- Mean: ?
- Multi-step transitions are also Gaussians
	- We can efficiently compute the noisy distribution at every step directly, without computing all the previous ones

Diffusion perspective
- Heat diffusion
- This process defined by transition kernel which is Gaussian
	- What is transition kernel?
	- https://chatgpt.com/c/695e9562-e674-8331-a879-2319a4d41bd6
	- A transition kernel describes how a random process moves from one state to the next
	- A transition kernel is the conditional probability distribution that defines how a stochastic process moves from one step(state?) to the next

Iterative denoising
- Reverse diffusion process
- Issue: Denoising distribution is unknown
- Solution: Learn a variational approximation
- There is the true denoising distribution, we are approximating it with some Gaussian.
- Joint distribution
- Mean of the denoising distribution is basically the score
	- TODO
	- mean = x' - noise

Hierarchical VAE
- ELBO formula
	- https://chatgpt.com/c/695ea621-3084-832e-ac3e-3ff85c866612
- ELBO loss reduces to denoising score matching
	- We need to parameterize the predicted mean in a certain way: mean = x' - noise
	- Can we do another parametrization?
		- TODO
- vs. Score based model
	- sampling procedure is different
		- what is the difference?
		- what is the difference with one-step Langevin?
		- just scaling?
	- loss is the same

Hierarchical VAE vs. Denoising score matching
- https://chatgpt.com/c/69613627-42a0-8326-a3c6-18430b1b8529
- Связь с DDPM sampling
	- Euler–Maruyama шаг SDE, который эквивалентен одному шагу Langevin для $p_t(x)$
- ALD естественно даёт trade-off
	- Модель та же → меняется качество численного решения SDE.

You need VAE recap!
- [x] Done 1 time

You need Langevin dynamics recap
- [[Langevin MCMC]]
- TODO: implement
- Putting more compute comparing to just sampling for Gaussian
	- Several steps to do the sampling?

Architecture for the denoiser
- UNet
- UNet itself has VAE like architecture


Predictor-Corrector
- Different method of solving the same underlying SDE
- DDPM is just Predictor
- Score based models is just Corrector
...
- https://chatgpt.com/c/694bb364-740c-8325-87df-c4fd67e9e9ca
- Метод Эйлера
- Diffusion ODE (она же probability flow ODE)
- Идея:  найти детерминистическое ОДУ по времени вперёд (без шума), у которого маргинальные распределения по времени такие же, как у forward SDE.

Converting SDE to ODE
- Now the Forward ODE depends on the score
- Convert this model to NF
- Unique ODE solution -> Invertible mapping
- Converted VAE to Flow model
- ODE and NF
